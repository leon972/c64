...
1090 ww%=102
1092 rem stato oggetti temporanei
1093 dim st(2):rem idx 1 messaggio di testo on/off 2 bonus on/off
1095 dim tc(2):rem contatori timer oggetti temp.
1098 rem bonus corrente 
1100 ba=0:bb=0:rem ba=ind. mem video bonus,bb=carattere bonus corrente
1105 rem inizializza il chip SID
1110 gosub 4310
1120 rem
1200 goto 2000
1205 rem worms subs
...
2702 gosub 4130:rem controlla oggetti temp.
2703 gosub 4220:rem gen bonus
...
3115 lv=lv-1:i$="":ld=2:gosub 4510
3120 gosub 1240
...
3502 t$="level completed!":tw=60*3:gosub 4050
3505 gosub 3950:rem bonus punteggio in base a lunghezza
3510 wl%=3
3515 l1%=l1%+1
...
3542 if lv<5 then lv=lv+1:gosub 4480
3545 nl=nl+nl:gosub 1240:rem aggiorna vite
3546 t$="1up !":tw=60*3:gosub 4050
3550 return
3560 rem level 4
...
3950 rem *** bonus worm length ***
3960 rem cancella il verme a partire dalla coda dopo l'urto
3962 rem incrementa il punteggio ogni volta che elimina un elemento
3970 ic=ih%+1
3980 if ic>=wl% then ic=0
3990 i=ic:hf=27:lf=0:
4000 poke w1(i),32:tw=0:gosub 1370
4005 sc=sc+2:gosub 3460:lf=lf+10:if lf>255 then lf=0:hf=hf+1
4007 gosub 4350:poke s+4,32
4010 if i=ih% then return
4020 i=i+1
4030 if i>=wl% then i=0
4040 gosub 4130:goto 4000
4050 rem *** display temp message ***
4060 rem tw=tw=numero di sessantesimi di secondo, t$=messaggio
4070 st(1)=1:tc(1)=time+tw
4075 c=1:y=24:gosub 325:return
4080 rem *** display bonus ***
4085 rem tw=tempo in sessantesimi di sec. di visualizz. bonus
4090 rem rem ch=codice poke carattere,c=colore
4100 st(2)=1:tc(2)=time+tw
4110 tu%=fo%:fo%=ch:c=7:gosub 1287
4115 fo%=tu%:ba=a:bb=ch:rem memorizza posizione e carattere bonus
4120 return
4130 rem *** clear temp object ***
4135 rem rimuove gli oggetti temporanei
4140 if st(1)=1 then gosub 4160
4145 if st(2)=1 then gosub 4190
4150 return
4160 rem rimuove testo temp
4170 if time<tc(1) then return
4180 st(1)=0:t$="                                ":y=24:gosub 325
4185 return
4190 if time<tc(2) then return
4195 st(2)=0
4200 if peek(ba)=bb then poke ba,32:rem cancella bonus
4210 return
4220 rem *** gen bonus ***
4223 b%=rnd(0)*5:if b%<>1 then return
4225 if st(2)=1 then return
4230 b%=rnd(0)*20
4232 if b%=1 then goto 4270
4240 if b%=2 then goto 4270
4250 if b%=3 then goto 4270
4260 return
4270 c=7:tw=8*60:ch=88:gosub 4080
4280 return
4290 rem *** bonus 88 ***
4295 sc=sc+10:t$="+10 points!":tw=60*2:gosub 4050:gosub 3460:gosub 4460
4300 return
4305 rem musica ed effetti sonori
4306 rem il cip SID contiene 29 registri ad 8 bit
4307 rem posizionati in memoria da 54272 a 54296
4310 rem *** inizializza il SID ***
4315 let s=54272:rem indirizzo memoria sid
4320 for sm=s to s+24:poke sm,0:next
4330 return
4350 rem *** play frequency ***
4360 rem suona una frequenza hf=bit alto della frequenza lf=bit basso
4380 rem
4390 poke s+5,9:poke s+6,0
4400 poke s+24,15: rem set maximum volume level
4410 rem
4420 poke s+1,hf:poke s,lf
4430 poke s+4,33
4440 return 
4450 rem *** eat food sound ***
4455 hf=25:lf=170:gosub 4350:poke s+4,32:return
4460 rem *** bonus sound ***
4470 hf=25:lf=170:gosub 4350:hf=15:lf=170:gosub 4350:hf=24:lf=100:gosub 4350:poke s+4,32:return
4480 rem *** vincita vita sound***
4490 hf=26:for rt=1 to 100 step 10:lf=rt:gosub 4350:next:poke s+4,32
4500 return
4510 rem *** urto con muro sound***
4520 hf=19:lf=170:gosub 4350:for rt=1 to 80:next:poke s+4,32:return
